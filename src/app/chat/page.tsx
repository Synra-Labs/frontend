...
function getSessionId(): string {
  ...
  if (!v) { v = (crypto?.randomUUID?.() || `${Date.now()}-${Math.random()}`).toString(); localStorage.setItem(k, v); }
  return v;
}
...
export default function ChatPage() {
  ...
  const handleSend = async () => {
    ...
    if (!effectiveModelId) { setError("Please select or type a model ID."); return; }
    ...
    try {
      const { data } = await axios.post(`${apiBase}/run`,
  ...
  const handleClear = () => { setChatHistory([]); setError(""); setSource(""); };
    ...
    <div className="chat-page container">
      ...
      {!selectedModel && (
        <div className="mb-20" style={{ width: "100%", maxWidth: 800 }}>
          <input className="input" placeholder="Model ID (ft-... or HF id, e.g. TinyLlama/TinyLlama-1.1B-Chat-v1.0)"
            value={modelIdOverride} onChange={e => setModelIdOverride(e.target.value)} />
        </div>
      )}
      ...
      {source && <div className="mt-20" style={{ fontSize: 12, opacity: 0.7 }}>engine: <code>{source}</code></div>}
