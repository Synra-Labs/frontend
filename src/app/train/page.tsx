...
export default function TrainPage() {
  ...
  const [baseModelId, setBaseModelId] = useState(
    selectedModel || "TinyLlama/TinyLlama-1.1B-Chat-v1.0"
  );
  ...
  const startFakeProgress = () => {
    ...
    const interval = setInterval(() => {
      setProgress((p) => {
        const next = Math.min(95, p + Math.random() * 7);
        return next;
      });
    }, 900);
  ...
  const handleTrain = async () => {
    ...
    if (!files || files.length === 0) {
      alert("Please select training files (text).");
      return;
    }
    ...
    try {
      ...
      if (data?.job_id) {
        ...
        setIsTraining(false);
        setStatus("failed");
        setError("Training did not start.");
      }
    } catch (e: any) {
      cleanup();
      setIsTraining(false);
  ...
  const pollProgress = async (jid: string, cleanup: () => void) => {
    try {
      const { data } = await axios.get(`${apiBase}/progress/${jid}`, {
        headers: { "X-Session-Id": sessionId },
      });
      ...
      if (data?.status === "completed") {
        ...
        setFtModelId(mid);
        alert("✅ Training complete! Your model is ready.");
      ...
      // keep polling a couple more times
  ...
  const handleDownloadModel = async () => {
    ...
    try {
      const resp = await fetch(
        `${apiBase}/download-model?model_id=${encodeURIComponent(ftModelId)}`,
        { headers: { "X-Session-Id": sessionId } }
      );
      if (!resp.ok) return alert("Model not ready to download.");
      const blob = await resp.blob();
      const a = document.createElement("a");
      a.href = URL.createObjectURL(blob);
      a.download = `${ftModelId}.zip`;
      a.click();
  ...
  const handleLoadAndChat = async () => {
    if (!ftModelId) return;
    try {
      // Preload into RAM (optional)
      await axios.get(`${apiBase}/load-local?model_id=${encodeURIComponent(ftModelId)}`, {
        headers: { "X-Session-Id": sessionId },
      });
    } catch {
      // non-fatal; /chat will lazy-load if needed
    }
    setSelectedModel(ftModelId);
    router.push("/modify"); // go through modify so user can tune settings, then chat
  };
  ...
  return (
    <div className="train-page container">
      ...
      <div className="train-form">
        <label>Base Model (HF repo or your FT id):</label>
        <input
          type="text"
          value={baseModelId}
        ...
        <label>Dataset Files (text):</label>
        <input
          type="file"
          multiple
        ...
        {status === "completed" && ftModelId && (
          <div style={{ marginTop: 16, display: "flex", gap: 10, flexWrap: "wrap" }}>
            <button onClick={handleLoadAndChat} className="btn train-btn">
              Load & Chat ({ftModelId})
            </button>
            <button onClick={handleDownloadModel} className="btn train-btn">
              Download Model (.zip)
            </button>
          </div>
        )}
        ...
        {error && (
          <div className="success-banner" style={{ background: "#402727", color: "#ffdcdc", marginTop: 16 }}>
            {error}
          </div>
        )}
      </div>
